...
public abstract class BrowserEditorPart extends EditorPart {
	...
	public void createPartControl(Composite parent) {
		...
		browser.addCloseWindowListener(new CloseWindowListener() {
			public void close(WindowEvent event) {
				isDirty = false;
				getEditorSite().getPage().closeEditor(BrowserEditorPart.this, true);
			}
		});
		...
		browserRPC.addRPCHandler("status", new BrowserRPCHandler() {
			public Object handle(Object[] args) {
				if ("setMessage".equals(args[1])) {
					IStatusLineManager slm = getEditorSite().getActionBars().getStatusLineManager();
	...
	protected BrowserEditorPart openWindow(WindowEvent event) {
		...
		try {
			view = (BrowserEditorPart)getEditorSite().getPage().openEditor(
					getNewWindowEditorInput(event),getNewWindowEditorId(event));
